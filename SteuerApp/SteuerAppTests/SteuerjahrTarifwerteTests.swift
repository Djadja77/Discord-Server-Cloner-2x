...
/// Hält die hinterlegten Tarifkonstanten auf den Werten fest, die im Gesetz stehen.
///
/// `SteuerjahrKonsistenzTests` prüft, ob die Zahlen **zueinander** passen - dieser Test
/// prüft, ob es **die richtigen** Zahlen sind. Beides zusammen fängt sowohl Tippfehler
/// als auch stilles Abdriften ab: wer hier eine Zahl ändert, muss den Test mitändern und
/// stolpert dabei über die Quellenangabe.
///
/// Quellen:
/// - § 32a Abs. 1 EStG in den Fassungen für die Veranlagungszeiträume 2024, 2025 und 2026
/// - Amtliches Einkommensteuer-Handbuch des Bundesfinanzministeriums (für 2025)
/// - § 3 SolZG für die Freigrenzen des Solidaritaetszuschlags
/// - § 32 Abs. 6 EStG für die Kinderfreibeträge
final class SteuerjahrTarifwerteTests: XCTestCase {
    ...
    func testKinderfreibetraegeUndKindergeld() {
        // Beträge für beide Elternteile zusammen.
    ...
    func testHoechstbetraegeDerAltersvorsorge() {
        ...
        XCTAssertEqual(Steuerjahr.jahr2024.höchstbetragAltersvorsorge, 27_566)
        XCTAssertEqual(Steuerjahr.jahr2025.höchstbetragAltersvorsorge, 29_344)
        XCTAssertEqual(Steuerjahr.jahr2026.höchstbetragAltersvorsorge, 30_826)
    }

    func testAlleJahreSindAlsGeprueftMarkiert() {
        for jahr in Steuerjahr.alle {
            XCTAssertTrue(jahr.amtlichGeprüft,
                          "Jahr \(jahr.jahr) ist nicht als geprüft markiert")
        }
    }
