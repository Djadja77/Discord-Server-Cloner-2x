...
///
/// Das Ergebnis ist ein **Vorschlag**, kein Ersatz für die Kontrolle. Die Werte landen in
/// der Maske und können dort korrigiert werden. Erkannt wird, was deutsche Belege
/// üblicherweise zeigen - bei einem zerknitterten Tankbeleg bleibt eben ein Feld leer.
enum BelegTexterkennung {

    struct Vorschlag: Equatable {
        var bruttoBetrag: Decimal? = nil
        var datum: Date? = nil
        var händler: String? = nil
    ...
    /// Zeilen, die zwar oben stehen, aber nie der Händlername sind.
    private static let keinHändler = [
    ...
    /// Straßenbezeichnungen. Zusammen mit einer Hausnummer kennzeichnen sie eine
    /// Adresszeile - "Bäckerei Sonnenweg" ohne Ziffer bleibt dagegen ein Händlername.
    private static let straßenworte = ["str.", "strasse", "straße", "platz", "allee", "weg", "gasse"]
    ...
    ///
    /// Von der Bilderkennung getrennt, damit die Auswertung ohne Kamera und ohne Gerät
    /// prüfbar bleibt - dort sitzen die Fehler, nicht in der Texterkennung selbst.
    static func auswerten(zeilen: [String]) -> Vorschlag {
        let betrag = betragFinden(in: zeilen)
        let händler = händlerFinden(in: zeilen)
        return Vorschlag(
            bruttoBetrag: betrag,
            datum: datumFinden(in: zeilen),
            händler: händler,
            umsatzsteuersatz: steuersatzFinden(in: zeilen, bruttoBetrag: betrag),
            kategorie: Kategorievorschlag.fuer(händler: händler, zeilen: zeilen),
    ...
    static func zeilenLesen(bild: UIImage) async -> [String] {
        ...
        return await withCheckedContinuation { fortsetzung in
            let anfrage = VNRecognizeTextRequest { anfrage, _ in
                ...
                // Von oben nach unten sortieren: der Händler steht im Kopf des Belegs,
                // und die Reihenfolge entscheidet daher über die Trefferqualität.
    ...
    /// Sucht zuerst in Zeilen mit Summenwort, sonst den größten Betrag des Belegs.
    static func betragFinden(in zeilen: [String]) -> Decimal? {
        ...
        if let treffer = mitSummenwort.flatMap(beträge(in:)).max() {
            return treffer
        }
        return zeilen.flatMap(beträge(in:)).max()
    }

    /// Alle Geldbeträge einer Zeile - deutsche Schreibweise mit Komma als Dezimaltrenner.
    static func beträge(in zeile: String) -> [Decimal] {
    ...
    /// Der Händlername steht im Kopf des Belegs - allerdings selten in der ersten Zeile.
    static func händlerFinden(in zeilen: [String]) -> String? {
        for zeile in zeilen.prefix(6) {
            ...
            guard !keinHändler.contains(where: { klein.contains($0) }) else { continue }

            // Betragszeilen aussortieren: überwiegend Buchstaben muss es sein.
    ...
    /// Straße mit Hausnummer oder Postleitzahl - beides gehört zur Anschrift, nicht zum Namen.
    static func istAdresszeile(_ zeile: String) -> Bool {
        let klein = zeile.lowercased()
        let hatZiffer = zeile.contains { $0.isNumber }
        if hatZiffer, straßenworte.contains(where: { klein.contains($0) }) { return true }
    ...
    /// Quittung mit Speisen und Getränken etwa -, wird geprüft, welcher Satz zum
    ...
    static func steuersatzFinden(
        ...
        let sätze = prozentangaben(in: zeilen)

        if sätze == [19] { return .regel }
        if sätze == [7] { return .ermaessigt }
        guard sätze.contains(19), sätze.contains(7), let brutto = bruttoBetrag,
              brutto > 0 else { return nil }

        // Prozentangaben vorher entfernen: "19,00 %" würde sonst als Betrag von 19,00 Euro
        // gelesen und die Prüfung auf sich selbst zurückführen.
        let ohneProzente = zeilen.map {
            $0.replacingOccurrences(of: #"\d{1,2}(?:[.,]\d{1,2})?\s*%"#, with: " ",
                                    options: .regularExpression)
        }
        let vorhandeneBeträge = Set(ohneProzente.flatMap(beträge(in:)))
        let passend = [Umsatzsteuersatz.regel, .ermaessigt].filter {
            vorhandeneBeträge.contains($0.steueranteil(ausBrutto: brutto))
        }
